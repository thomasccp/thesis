...
As mentioned above, fast and efficient PQ is a pre-requisite for effective navigation through access routes to the target anatomy~\cite{kwok10}.
...
%\subsection{Geometric Representation for PQ}
...
%\subsection{Hardware Acceleration for PQ}
...
%and also demonstrate how it can be well-incorporated with FPGA. 

To leverage the advantages of FPGAs for hardware acceleration, Chow et al.~\cite{chow11} proposed a mixed precision methodology.
...
%The PQ is proceeded effectively to calculate the shortest distance only between a point and the corresponding edge of the polygon. 
...
The PQ formulation sketched in the previous section is not entirely hardware-friendly.
...
\REPEAT
	...
	\STATE {Configure the FPGA with precision $p_{test}$}
	\STATE {Compute PQ and get $T(p_{test})$}
\UNTIL{$T(p_{test}) > T(p_L)$}
\end{algorithmic}
\label{algo:reconfig}
\end{algorithm}

%Reconfiguration requires stopping the whole or a part of an FPGA for loading a new bit-stream.
%On a reconfigurable accelerator platform with multiple FPGAs,
%we can reconfigure one FPGA while the other FPGAs are still processing and hence the system would not stop operating during the precision-tuning process.
...
%For the FPGA-based implementation, the double-precision design is too large to fit into the FPGA chip.
%Thus, we assume the implementation is realised on a larger FPGA and the results are obtained by the performance model in Section~\ref{sec:implementation}.
