...
\FOR{each step $t$}
	...
	\WHILE{$idx1 \leq itl\_outer$}
		...
		\WHILE{$idx2 \leq itl\_inner$}
			\STATE{Sample a new state $\{\widetilde{\chi}_{t+1}^{(i)}\}^{P_t}_{i=1}$ from $\{\chi_{t}^{(i)}\}^{P_t}_{i=1}$} \label{algo:smc_si1_ch5}
			\STATE{Calculate unnormalised importance weights $\{\widetilde{w}^{(i)}\}^{P_t}_{i=1}$ and accumulate the weights as $w_{sum}$} \label{algo:smc_si2_ch5}
			\STATE{$idx2 \gets idx2 + 1$}
		\ENDWHILE
		\STATE{Calculate the lower bound of sample size $\widetilde{P}_{t+1}$ by Equation~\ref{eqt:bound2b_ch5}} \label{algo:smc_lb_ch5}
	{\color{gray} \STATE{---On \gls{cpu}s---}}
	\STATE{Sort $\{\widetilde{\chi}_{t+1}^{(i)}\}^{P_t}_{i=1}$ in descending $\{\widetilde{w}^{(i)}\}^{P_t}_{i=1}$} \label{algo:smc_sort_ch5}
	\IF{$\widetilde{P}_{t+1} < P_{t}$} \label{algo:smc_rd1_ch5}
		...
		\FOR{$i$ in $P_t-P_{t+1}$} \label{algo:smc_loop1s_ch5}
	...
	\ELSIF{$\widetilde{P}_{t+1} \geq P_{t}$} \label{algo:smc_rd3_ch5}
		\STATE{$a=0$ and $b=0$}
	...
	\STATE{Resample $\{\widetilde{\chi}^{(i)}_{t+1}\}^{P_t}_{i=1}$ to $\{\chi^{(i)}_{t+1}\}^{P_{t+1}}_{i=1}$} \label{algo:smc_r_ch5}
	\ENDIF
	\ENDWHILE
	\STATE{Update}
\ENDFOR
