...
$BW_{bus}$ is the bandwidth of the bus connecting the \gls{cpu} to \gls{fpga}.
...
$N_{P_{t+1}}$ describes the lower bound of particle size which is used in Algorithm~\ref{algo:smcb}:
...
\FOR{each step $t$}
	...
	\WHILE{$idx1 \leq itl\_outer$}
		...
		\ENDWHILE
		\STATE{Calculate the lower bound of sample size $\widetilde{N_{P_{t+1}}}$ by Equation~\ref{eqt:bound2b_ch5}} \label{algo:smc_lb_ch5}
	...
	\IF{$\widetilde{N_{P_{t+1}}} < {N_{P_t}}$} \label{algo:smc_rd1_ch5}
	...
	\ELSIF{$\widetilde{N_{P_{t+1}}} \geq {N_{P_t}}$} \label{algo:smc_rd3_ch5}
		\STATE{$a=0$ and $b=0$}
...
A surrogate model is employed to enable rapid learning of the valid design space and to deal with a large number of parameters.
