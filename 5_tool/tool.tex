...
%which uses Java as the underlying language. 
...
Assume that the \gls{cpu} is dedicated to particle set resizing and resampling, and the computation time scales linearly with the data size.
...
\begin{equation}
\begin{aligned}
\widetilde{N}_{P_{t+1}} = \sigma^2 \cdot \frac{{N_{P_{max}}}}{Var(\{\widetilde{\chi}_{t+1}^{(i)}\}^{{N_{P_t}}}_{i=1})} \mbox{.}
\end{aligned}
...
\FOR{each step $t$}
	...
	\WHILE{$idx1 \leq itl\_outer$}
		...
		\WHILE{$idx2 \leq itl\_inner$}
			\STATE{Sample a new state $\{s_{t+1}^{\prime(i)}\}^{N_{P_t}}_{i=1}$ from $\{s_t^{(i)}\}^{N_{P_t}}_{i=1}$} \label{algo:smc_si1_ch5}
			\STATE{Calculate unnormalised importance weights $\{w^{\prime(i)}\}^{N_{P_t}}_{i=1}$ and accumulate the weights as $w_{sum}$} \label{algo:smc_si2_ch5}
			\STATE{$idx2 \gets idx2 + 1$}
		\ENDWHILE
		\STATE{Calculate the lower bound of sample size $\widetilde{N}_{P_{t+1}}$ by Equation~\ref{eqt:bound2b_ch5}} \label{algo:smc_lb_ch5}
	{\color{gray} \STATE{---On \gls{cpu}s---}}
	\STATE{Sort $\{s_{t+1}^{\prime(i)}\}^{N_{P_t}}_{i=1}$ in descending $\{w^{\prime(i)}\}^{N_{P_t}}_{i=1}$} \label{algo:smc_sort_ch5}
	\IF{$\widetilde{N}_{P_{t+1}} < {N_{P_t}}$} \label{algo:smc_rd1_ch5}
		\STATE{${N_{P_{t+1}}} = max\left(\lceil\widetilde{N}_{P_{t+1}}\rceil, {N_{P_t}}/2\right)$}
		\STATE{Set $a = 2{N_{P_{t+1}}}-{N_{P_t}}$ and $b = {N_{P_{t+1}}}$}
		{\color{gray} \STATE{--Do the following loop in parallel--}}
		\FOR{$i$ in ${N_{P_t}}-{N_{P_{t+1}}}$} \label{algo:smc_loop1s_ch5}
		...
		\ENDFOR \label{algo:smc_rd2_ch5} \label{algo:smc_loop1e_ch5}
	\ELSIF{$\widetilde{N}_{P_{t+1}} \geq {N_{P_t}}$} \label{algo:smc_rd3_ch5}
		\STATE{$a=0$ and $b=0$}
	...
	\STATE{Resample $\{s_{t+1}^{\prime(i)}\}^{{N_{P_t}}}_{i=1}$ to $\{s_{t+1}^{(i)}\}^{N_{P_{t+1}}}_{i=1}$} \label{algo:smc_r_ch5}
	\ENDIF
	\ENDWHILE
	\STATE{Update}
\ENDFOR
...
%The design space has two dimensions: $N_P$ and $S$ (Table~\ref{tab:parameters}).
...
%Note that the data size being processed is very small, therefore the run-time per step is dominated by two factors: (1) the overhead of invoking the \gls{fpga} kernels, (2) initiating the data transfer to \glspl{cpu} and external memory via PCI Express and \gls{dram} controller.
...
%In the next two sections, two bigger applications which justify using larger data sets are evaluated.

%\todo[inline]{Is there any situation where the usage of a large number of particles can be justified?}
	...
	\begin{threeparttable}
		\begin{tabular}{c|p{6cm}|ccgc}
			\hline
															...
															&											& w/ opt.~\tnote{a}		& w/ opt.~\tnote{b} 	& w/ opt.~\tnote{c} & w/o opt.~\tnote{d} 	\\
			\hline
			\hline
