...
\FOR{each step $t$}
	...
	\STATE{Calculate the lower bound of sample size $\widetilde{N}_{P_{t+1}}$ by Equation~\ref{eqt:bound2}} \label{algo:smc_lb}
	...
	\IF{$\widetilde{N}_{P_{t+1}} < N_{P_t}$} \label{algo:smc_rd1}
	...
	\ELSIF{$\widetilde{N}_{P_{t+1}} \geq {N_{P_t}}$} \label{algo:smc_rd3}
		\STATE{$a=0$ and $b=0$}
...
\begin{equation}
\begin{aligned}
%T_{config} = \frac{\mathit{size}_{bs}}{\mathit{freq}_{config} \cdot W_{config}} \mbox{,}
...
%$\mathit{freq}_{config}$ is the configuration clock frequency in Hz and $W_{config}$ is the bit-width of the configuration port.

\setcounter{subfigure}{0}
\begin{figure}[t!]
\centering
\subfigure[Without reconfiguration]{
\includegraphics[width=0.7\textwidth]{4_adaptation/figures/fig_timing1} \hfill
\label{fig:timing1}
}
