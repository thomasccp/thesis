...
\FOR{each step $t$}
	...
	\STATE{Sample a new state $\{s_{t+1}^{'(i)}\}^{P_t}_{i=1}$ from $\{s_t^{(i)}\}^{P_t}_{i=1}$} \label{algo:smc_si1}
	\STATE{Calculate unnormalised importance weights $\{w^{'(i)}\}^{P_t}_{i=1}$ and accumulate the weights as $w_{sum}$} \label{algo:smc_si2}
	\STATE{Calculate the lower bound of sample size $\widetilde{P}_{t+1}$ by Equation~\ref{eqt:bound2}} \label{algo:smc_lb}
	{\color{gray} \STATE{---On \glspl{cpu}---}}
	\STATE{Sort $\{s_{t+1}^{'(i)}\}^{P_t}_{i=1}$ in descending $\{w^{'(i)}\}^{P_t}_{i=1}$} \label{algo:smc_sort}
	\IF{$\widetilde{P}_{t+1} < P_{t}$} \label{algo:smc_rd1}
		...
		\FOR{$i$ in $P_t-P_{t+1}$} \label{algo:smc_loop1s}
			\STATE{$s_{t+1}^{'(i)} = \frac{s_{t+1}^{'(a)} w^{'(a)} + s_{t+1}^{'(b)} w^{'(b)}}{w^{'(a)} + w^{'(b)}}$}
			\STATE{$w^{'(i)} = w^{'(a)} + w^{'(b)}$}
	...
	\ELSIF{$\widetilde{P}_{t+1} \geq P_{t}$} \label{algo:smc_rd3}
		...
		\FOR{$i$ in $P_{t+1}-P_{t}$}
			\IF{$w^{'(a)}<w^{'(a+1)}$ and $a<P_{t+1}$}
				\STATE{$a=a+1$}
			\ENDIF
			\STATE{$s_{t+1}^{'(P_{t}+b)} = s_{t+1}^{'(a)}/2$}
			\STATE{$s_{t+1}^{'(a)} = s_{t+1}^{'(a)}/2$}
			\STATE{$w^{'(P_{t}+b)} = w^{'(a)}/2$}
			\STATE{$w^{'(a)} = w^{'(a)}/2$}
			\STATE{$b=b+1$}
		\ENDFOR
	\ENDIF \label{algo:smc_rd4}
	\STATE{Resample $\{s_{t+1}^{'(i)}\}^{P_t}_{i=1}$ to $\{s_{t+1}^{(i)}\}^{P_{t+1}}_{i=1}$} \label{algo:smc_r}
	\STATE{$r=r+1$}
	\ENDWHILE
\ENDFOR
\end{algorithmic}
...
At the initial time-step ($t=0$), the number of particles $P_0$ is initialised with $P_{max}$ which is the maximum number of available particles.
...
where the clock frequency and number of threads of the \glspl{cpu} are represented by $f_{\gls{cpu}}$ and $N_{thread}$ respectively.
