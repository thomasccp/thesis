...
\gls{fpga} architectures implement digital circuits by providing numerous fine-grained resources, namely \glspl{lut} implemented in small RAMs.
...
\glspl{fpga} enable sequential circuits by providing \glspl{ff} along \gls{lut} outputs.
By providing hundreds of thousands of \glspl{lut} and \glspl{ff}, \glspl{fpga} can implement massively parallel circuits.
...
Designers can also use the \glspl{lut} to implement soft processors, such as Xilinx's MicroBlaze~\cite{xilinxmicroblaze} and Altera's Nios II~\cite{alteranios2}.

To combine these resources into larger circuits, \glspl{fpga} provide reconfigurable interconnect.
In between each row and column of resources, \glspl{fpga} contain numerous routing tracks, which are wires that carry signals across the chip.
Connection boxes provide programmable connections between resources I/O and routing tracks.
Switch boxes provide programmable connections between intersecting routing tracks.
Such programmable connections allow a signal to be routed to any destination on the \gls{fpga} chip.
This architecture is called an island-style fabric as show in Figure~\ref{fig:bg_islandstyle}.
...
The reconfigurability of ~\glspl{fpga} leads to an unique technique which allows circuitry to be updated selectively on the fly, without disturbing the execution of the remaining system.
...
To implement applications on \glspl{fpga}, \gls{fpga} tools generally take a design flow as shown in Figure~\ref{fig:bg_flow}.
...
To implement a region of code on an FPGA, there are two major programming models.
The most common model is to manually convert the code into a semantically equivalent \gls{rtl} circuit, which designers typically specify using \glspl{hdl} such as VHDL and Verilog.
...
Such complexity leads researcher to work on \gls{hls} tools.
Commercial \gls{hls} tools are becoming increasingly common.
Example includes Xilinx's Vivado HLS~\cite{xilinxvivado}, Impulse Accelerated Technologies' Impulse C~\cite{impulsec}, Calypto's Catapult C~\cite{catapultc}, Mentor Graphics' Handle-C~\cite{handelc}, IBM's Lime~\cite{lime}, Bluespec~\cite{bluespec}, OpenCL in Altera~\cite{alteraopencl}, and Matlab's support of Altera and Xilinx \glspl{fpga} through DSP Builder~\cite{alteradspbuilder} and System Generator~\cite{xilinxsysgen}.
Open-source tools such as LegUp~\cite{legup} are also gaining researchers' attention.

OpenSPL is a \gls{hls} that has recently been proposed~\cite{openspl}.
It is a spacial programming language focusing on data-flow computing.
MaxCompiler~\cite{maxcompiler} is a commercial tool which supports OpenSPL and is developed by Maxeler Technologies.
MaxCompiler also supports a series of reconfigurable \gls{hpc} systems.
The set of integrated compilation tool and systems address two problems:
a) FPGA applications are specific to one product, designers targeting a different product must often make significant changes to an RTL circuits.
Low-level details, device specific, such as controllers for DDR memory, PCI Express and Ethernet are abstracted and optimised.
It also allows porting of applications to different systems via the same tool package.
b) Designers generally debug FPGA applications by simulating the RTL code or technology-mapped circuit and analysing hundreds of waveforms.
Maxeler tools provide cycle accurate simulator for the FPGA kernel.
...
designers are still suffering from long synthesis time which makes design space exploration very inefficient.
Traditional software techniques that relies on rapid recompilation are no longer feasible.
...
In dealing with large design space, an optimisation algorithm~\cite{kurek13arc} is developed based on Efficient Global Optimisation (EGO)~\cite{jones98}. 
...
Nowadays, \glspl{fpga} are programmed in \gls{rtl} using Verilog or VHDL. 
...
\gls{rtl} design is also error prone and non-portable, therefore, \glspl{dsl} or \glspl{dcl} are being promoted to increase programmer productivity and code quality.
...
A real-time system is one that must process information and produce a response within a specified time, otherwise severe consequences, including failure, can result.
A system is defined as being real-time if it is required to respond to an input stimuli within a finite and specified time interval.
The stimuli is either an event at the interface to the system or some internal signals.
A real-time system is one whose correctness is based on both the correctness of the outputs and their timeliness, but the system does not have to be fast.
A hard real-time system should response to events within fixed time.
...
\end{center}
\caption[Various sets of vertex points aligned on a series of contours; A set of vertex points located on an arbitrary form of mesh.]{(Left) Various sets of vertex points aligned on a series of contours; 
...
One drawback of \gls{smc} is its long execution times that limit its practical use.
...
This process is described in Algorithm~\ref{algo:smc}.
...
\begin{enumerate}
\item \textbf{Initialisation}: Weights $\{w_t^{(i)}\}^{N_P}_{i=1}$ are set to the same value, e.g. $\frac{1}{N_P}$.
...
The basic model is shown in Equation~\ref{eq:sv_model}, where $y_t$ is the observable time varying volatility and $s_t$ represents the stochastic log-volatility process.
...
\gls{smc} methods are applied to air traffic management~\cite{kantas09,chau13acm,eele13cdc,eele13gnc,lymperopoulos10,lymperopoulos10thesis}.
At discrete time intervals, control actions are determined by \gls{smc} and are applied to adjust aircraft trajectories.
...
The variables are subject to constraints: $V^i_{t} \subset [V_{min}, V_{max}]$, $M^i_{t} \subset [M_{min}, M_{max}]$, $T^i_{t} \subset [T_{min}, T_{max}]$, $\phi^i_{t} \subset [\phi_{min}, \phi_{max}]$, $\tau^i_{t} \subset [\tau_{min}, \tau_{max}]$.
...
These applications are accelerated by various reconfigurable technologies, which relate to the contributions of this thesis.
