...
This paper introduces a design flow for generating efficient implementation of reconfigurable SMC designs. Using it, users can develop efficient multiple-FPGA SMC applications without any knowledge of reconfigurable computing.
...
Experimental results on 3 case studies show that design performance is substantially improved after parameter optimisation. 
...
SMC methods are often preferable over Kalman filters and hidden Markov models, as they do not require exact analytical expressions to compute the evolving sequence of posterior distributions.
...
Although earlier publications in~\cite{chau13a,chau13b,happe13,hendeby07} attempted to implement SMC efficiently on FPGAs, design productivity remains a challenge.
...
For example, the performance of SMC relies on the a set of random samples, which are called particles in the following.
...
In this paper, we propose a SMC design flow for reconfigurable hardware.
...
\item The utility of this approach in terms of design productivity and performance is quantified over a diverse set of SMC problems.
...
SMC methods estimate the unobserved states of interest based on the given observations~\cite{doucet01}.
...
This process is described in Algorithm~\ref{algo:smc} and involves 5 stages of computation:
...
SMC methods are applied to model predictive control (MPC) optimisation where control actions at discrete time intervals are determined to minimise an error criteria~\cite{kantas09}.
...
For more details of the model, refer to ~\cite{eele13}.
...
designs. The design flow has 2 novel features to minimise hardware redesign efforts:
...
In this work the synthesis tool employed was Maxeler's MaxCompiler, which uses Java as the underlying language. 
MaxCompiler also supports a list of FPGAs such that low level configurations, such as I/O binding, are performed automatically.
...
Code~\ref{lst:def} illustrates the class \textit{Def} where number representation (floating-point, fixed-point with different bit-width), structs (state, reference), static parameters (Table~\ref{tab:parameters}) and system parameters are defined.
...
Given current state \textit{s\_in}, reference \textit{r\_in} and observation \textit{m\_in} (sensor value in this example), an estimation state \textit{s\_out} is computed.
...
The computation engine is fully pipelined and outputs 1 result per clock cycle.
...
The sampling and importance weighting process can be accelerated using multiple cores, in which each of them is responsible for a sub-part of the inner loop iterations or particles.
...
As we will see in the applications below, we are able to choose the optimal set of parameters without doing exhaustive search.
...
The design flow is first evaluated using a small-sized application on a microATX computer. The stochastic volatility model was described using the flow 
and targeted to a Xilinx Virtex-6 XC6VSX475T FPGA at 150 MHz.
...
The design space has 2 dimensions, $N_P$ and $S$.
...
This paper demonstrated the feasibility of generating highly-optimised reconfigurable designs for SMC applications, while hiding all aspects of reconfigurable computing from the user.  
...
We further established that a surrogate software model combined with machine learning can be used to rapidly optimise designs, reducing optimisation time from days to hours; 
...
We are currently investigating run-time optimisation of parameters based on our initial work in~\cite{chau13a}.
We will also automate the design flow to allow translation of popular software programming languages (e.g. R, MATLAB) directly to reconfigurable designs. 
