...
In this chapter, we propose an \gls{smc} design flow for reconfigurable hardware.
...
%These flexibilities minimise hardware redesign efforts and allow \gls{fpga}s to take advantage of the parameter tuning methodology in Section~\ref{sec:optimisation}.
...
\textbf{\gls{fpga} Func}: \textit{Sampling and importance weighting} (line~\ref{algo:s} and~\ref{algo:i} of Algorithm~\ref{algo:smc}) are the most computation intensive functions, and accelerated by \gls{fpga}s.
...
%It utilises a deep pipeline of \gls{fpga}s so that particles can be processed simultaneously.
...
Figure~\ref{fig:kernel} shows the design of the \gls{fpga} kernel. Blocks that require customisation are darkened.
...
\begin{table}[h]
	...
	\begin{threeparttable}
		\begin{tabular}{l|x{2.2cm}y{2.2cm}}
			\hline
															& \gls{cpu}~\tnote{a} 		& This work~\tnote{b} \\
			\hline
			\hline
...
%For example, in a future scenario with 50 aircraft and a reconfigurable platform with 24 \gls{fpga}s, design exploration indicates that
