...
Fast and efficient \gls{pq} is a pre-requisite to provide haptic guidance which allow the operator perform effective and safe surgery. 
...
In this chapter, we derive a \gls{pq} formulation which allows objects to be represented in complex geometry with vertices.
...
\begin{itemize}
\item A hardware-friendly \gls{pq} formulation for calculating the relative placement of objects modelled by vertices with complex morphology, which facilitates
...
Section~\ref{sec:precision_optimisation} discusses the optimisation of \gls{pq} for reconfigurable accelerator.
Section~\ref{sec:precision_implementation} describes the system design that maps \gls{pq} to a reconfigurable accelerator.
...
\begin{equation}
\begin{aligned}
^j\mu = \frac{(^jV_2-X)\cdot(^jV_3-^jV_2)}{||^jV_3-^jV_2||^2} \\
...
It is computed at run-time and the details will be discussed later in Section~\ref{sec:precision_search}.
...
$TH_{Comp,p_L}$ (Equation~\ref{eqt:fM}) is the run-time measured throughput when using precision $p_L$ as the reduced precision.
...
As shown in Fig.~\ref{fig:precision_stream}, data are streamed in an order as indicated by the arrows.
...
Fig.~\ref{fig:precision_arch} shows our proposed system architecture which consists of three major components.
...
Consider a group of $N_{S}$ points (i.e. $x_1, x_2, ..., x_{N_S}$) being processed together in the pipeline, we use a FIFO of length $N_{S}$ where
...
\end{center}
\caption[Computation time and the level of parallelism vs. different number of mantissa bits.]{Computation time [dotted line] and the level of parallelism [solid line] 
...
\end{center}
\caption[Ratio of re-computation and the number of points processed in 1 ms vs. different number of mantissa bits.]{Ratio of re-computation [dotted line] and 
...
\begin{table}[t!]
	\begin{spacing}{1.0}
	\caption[Comparison of PQ computation in 1 ms using CPU-based system, GPU-based system, double precision FPGA-based system and FPGA+CPU system with reduced precision]{Comparison of PQ computation in 1 ms using CPU-based system (CPU), GPU-based system (GPU), double precision FPGA-based system (FPGA DP) and FPGA+CPU system with reduced precision (FPGA RP)}
