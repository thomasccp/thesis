...
where ${^j\omega}^{\prime}_i$ is the two-dimensional mapping of $^j\omega_i$, $M_j=(M_{xj}, M_{yj}, M_{zj})$ is the tangent of centre line of contour $C_j$.
...
%the details will be discussed later in Section~\ref{sec:precision_search}.
...
\REPEAT
	...
	\STATE {Configure $FPGA_1$ with precision $p_{test}$, $FPGA_{2...N}$ are not reconfigured}
	\STATE {Compute \gls{pq} and get $TH_{comp,p_{test}}$}
\UNTIL{$TH_{comp,p_{test}} < TH_{comp,p_L}$}
%\FOR{$i$ in $2$ to $N$}
%	\STATE {Configure $FPGA_i$ with precision $p_{test}$}
%\ENDFOR
\end{algorithmic}
\label{algo:reconfig}
\end{algorithm}
...
%This is based on the assumption that the \gls{cpu} is dedicated to the computation of this application, and the computation time scales linearly with the data size.
...
\begin{table}[t!]
	...
	\begin{threeparttable}
		\begin{tabular}{l|c|c|c|g}
			...
			\hline
			Clock frequency (MHz) 			& 2,660 	& 1,150 			& 80 			& 130 \& 2,660 \tnote{a}  \\
			Number of cores				& 12		& 448				& 4 			& 20 \\
			\hline
			Number of mantissa bits		& 53		& 53				& 53			& 10 \& 53 \tnote{b} \\
...
The reconfigurable system design and performance model allow users evaluate system's real-time performance.
